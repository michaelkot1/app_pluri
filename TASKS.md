...
- [x] **M5-19** Apple Health connect reachable from Profile + Connected Apps (owner bug report, 2026-07-28 — SPEC §14 #78): M5-03's section/view model were never mounted, so nothing in the app ever called `LiveHealthKitService.requestAuthorization()` and steps/sleep were never authorized. Mount the real section on both screens, name unreadable categories after a partial grant, refresh Home/Insights without a relaunch, show today's value alongside the Insights 7-day average, and correct `NSHealthShareUsageDescription`.
...
> **Owner correction during M5-19 (2026-07-28) — the prompt, never Settings:** the first pass added an **Open Settings** row (`SystemSettingsLink` / `UIApplication.openSettingsURLString`) for the states where iOS won't re-prompt. Owner rejected it outright: "i dont want it taking me to settings" — the button must produce the native HealthKit sheet. `SystemSettingsLink.swift` is deleted (nothing else referenced it; the Notifications permission footer is text-only and needed no link) and `import UIKit` went with it. **Connect is now offered whenever HealthKit exists and access is incomplete**, not only at `.notDetermined`, and always calls `requestAuthorization` — HealthKit draws the sheet as long as any requested type is unasked, which holds on any device that only ever saw the workout HR/energy prompt. Complete, readable access is a bare "Connected" row; partial access reads "Partly connected". The unfixable case (a completed request that changed nothing) keeps Connect and switches the footer to plain-language guidance naming where Health access lives — **text only, no link** (SPEC §14 #78b/c).
...
> **Community posting follow-up (2026-07-29):** fixed the Debug root wiring to retain one live Supabase-backed client (fixtures remain preview/test-only), added posting/posted/error state coverage and a visible Posting… → Posted confirmation, and refined the hub, compose, create CTA, and post-card hierarchy with existing design tokens. This closes the create→feed identity gap found during device use; on-device visual/a11y spot-check remains recommended.
...
Injury-aware training (PLAN M10): move injury capture into the goal step as a binary gate plus a nested triage sub-flow, replace the 1–5 pain slider with a **qualitative** assessment (three primary exercises per injured area, characterized as **dull / sharp / aching**) that **derives** the existing 1–5 severity rather than replacing it, probe adjacent muscle groups, name injured users' training days for what they actually train, and ask "does this hurt / is it feeling better?" at three tiers across the life of the plan. Builds on M1's `OnboardingRouter`, M3's focus-first `PlanEngine` with graded pain rules (SPEC §14 #46/#47/#49), M3-15's reminder infrastructure, and M4's live logging + completion surfaces. Pluri is explicitly **not** physical therapy — the product goal is helping users manage pain and safely rebuild strength by training the surrounding musculature. **Exit:** an injured user is triaged qualitatively during onboarding without lengthening the flow for healthy users, receives a plan whose exercise selection reflects a derived severity and whose day names describe what that day actually is (never "Push / Pull / Legs"), is asked how flagged movements felt at the point of contact, and is re-assessed on a defined cadence that can adapt the remaining plan with their explicit confirmation.

### Scope & decisions (do first — unblock the capture model, schema, and engine)

- [ ] **M10-01** Resolve and record injury-aware training decisions in SPEC §14/§15 before building: the Q2 injury gate + nested triage sub-flow and the retirement of Q7 from the main flow; the **derived-severity** model where the qualitative assessment computes the existing `1...5` severity and the `profiles.injuries` jsonb grows as an **additive superset** (explicitly superseding the *capture model* of §14 #47/#49 while preserving their thresholds — add "superseded by" notes in place, per the precedent of #20); the sharp/aching/dull → severity mapping table; injury-aware day naming through the existing `plan_workouts.name` column plus injuries as an input to `SessionFocus.split`; the three-tier timing of "does this exercise hurt"; the four-layer re-assessment cadence and its dated-observation storage; and the non-medical framing / escalation posture. Also fix the live §3.2 Q7 inconsistency (it lists `Cardio` as an injury area, but `BodyArea` deliberately excludes it per §14 #15) and update §3.1's step count. Pick the most reversible interim option where the owner is unavailable (AGENTS §7). No Swift/SQL/UI in this task — M10-02+ implements against these decisions.

### Data model & persistence

- [ ] **M10-02** Injury assessment domain model + **additive** `profiles.injuries` jsonb superset (SPEC §14 #84): keep `area` and `pain` exactly as they are today (`[{ "area": "<BodyArea rawValue>", "pain": 1...5 }]`, §14 #31) so old clients, `OnboardingSyncMapper.hydrateProfile(from:)`, and the shipped `PlanEngine.role` path keep working unchanged, and add **optional** fields for pain quality, the provoking primary-exercise ids, the adjacent-area findings, an assessment date, and a schema version. Extend `InjuryJSON` in `Core/Sync/SyncDTOs.swift` with optional properties (rows written before M10 must still decode), and round-trip the new shape through `OnboardingSyncMapper.profileRow` / `hydrateProfile` + `OnboardingSyncMapperTests`. **No migration** — `profiles.injuries` is already jsonb.

- [ ] **M10-03** Migration for dated pain observations (SPEC §14 #88) — nothing in the schema stores a *dated* observation today, so re-assessment needs genuinely new tables: `injury_assessments` (id, user_id, `BodyArea` code, pain quality, derived severity, source tier, optional `plan_id` / `plan_workout_id`, `observed_at`, optional trend answer) and `injury_assessment_findings` (assessment_id, `workoutx_exercise_id`, response) — owner-only RLS on both, indexes on `(user_id, observed_at DESC)` and the FK, timestamps. Deliberately avoid the taken legacy prototype names (`workout_plans`, `plan_days`, `plan_day_exercises`, `user_equipment` — TASKS backlog / §14 #9). `profiles.injuries` stays the single current-state snapshot; these tables are the history behind it. No Swift client in this task.

### Onboarding: Q2 gate + nested triage sub-flow

- [ ] **M10-04** Injury gate on the goal step (SPEC §14 #83): `Q2GoalView` gains a binary "any active injuries right now?" question alongside the single-select `Goal` list (or as an immediately-adjacent branch off `.q2Goal`), writing a new `hasActiveInjuries` flag on `OnboardingAnswers`. A **no** answer advances straight to `.q3Experience` — healthy users see **zero** added steps and `OnboardingDestination.totalProgressSteps` does not grow for them. A **yes** answer pushes the M10-05 sub-flow. Default the gate to "no" per design.md's rule that required preference screens always have a preselected value.

- [ ] **M10-05** Nested injury triage sub-flow (SPEC §14 #83): a separate `InjuryTriageDestination` pushed onto the **same** `NavigationStack` as `OnboardingDestination`, with its own sub-progress indicator ("Injury check 2 of 4") and its own completion handler that returns to `.q3Experience`. It must **not** participate in `OnboardingDestination.rawValue` arithmetic, `next`, `stepNumber`, or the main progress denominator — renumbering the main enum would break `stepNumber`, the progress bar, SPEC §3.1's step count, and `AppRouterTests`. Back navigation out of the sub-flow returns to Q2 with the gate answer intact; abandoning the sub-flow (gate flipped back to "no") clears any partial assessment.

- [ ] **M10-06** Retire Q7 from the main flow (SPEC §14 #83): remove `.q7Injuries` from `OnboardingDestination`, delete `Q7InjuriesView`, drop `totalProgressSteps` from **14 → 13**, and re-check `stepNumber`'s rawValue-derived arithmetic after the case is removed. Update SPEC §3.1's step count and §3.2's Q7 entry (retired, pointing at the sub-flow — remaining questions keep their historical numbers per §14 #83d), update §14 #13 (the progress-bar denominator decision) in place, and fix `AppRouterTests` / `OnboardingSyncMapperTests` and any onboarding preview/fixture that walks the full enum. `InjuryPainRow` (the 1–5 `Slider` component) is superseded by the M10-09 qualitative screens — remove it and update **design.md §5.1**, whose "Injury severity (Q7)" bullet documents that slider.

- [ ] **M10-07** Affected-area picker inside the triage sub-flow: the standard coarse muscle-group list — `BodyArea`'s 9 cases (Back, Chest, Lower Arms, Lower Legs, Neck, Shoulders, Upper Arms, Upper Legs, Waist; `Cardio` stays excluded per §14 #15) — as a `PluriChipGrid` multi-select, reusing the existing chip presentation rather than inventing a new one. Selecting at least one area is required to continue; deselecting everything returns the user to the Q2 gate's "no" branch. Each selected area then gets its own M10-09 pass.

- [ ] **M10-08** Three-primary-exercise selection per area (SPEC §14 #87 Tier 1): a deterministic, seeded picker that returns exactly three well-known primary movements for a given `BodyArea` by filtering the local catalog on `targetMuscle` ∈ `BodyAreaMuscleMapping.muscles(for:)` (falling back gracefully when fewer than three qualify). Requires **no network call** — the catalog is a full local SwiftData mirror (~1,327 rows, `ExerciseCatalogStore`, refreshed at most every 7 days from the Supabase snapshot per §14 #25) — but `ExerciseCatalogStore` today exposes only `cachedExercises()` (fetches everything) and `exercise(id:)`, so add a **muscle-predicate read** and evaluate a `#Index` on `CachedExercise` for the muscle/body-part columns. Selection must be stable across relaunches for the same user + area (reuse the FNV-1a seed spirit of §14 #24) so the re-triage in M10-19 can ask about the *same* three exercises. Pure and unit-tested.

- [ ] **M10-09** Pain characterization screen (SPEC §14 #85, replaces the numeric slider): for each selected area, show the three M10-08 exercises with their existing cached media, ask **which of them provoke pain** (multi-select, "none of these" is a valid answer), and ask the user to characterize the pain as **dull**, **sharp**, or **aching** with plain, non-clinical one-line descriptions of each. Copy must be non-medical and non-scolding (design.md tone; AGENTS §5) and must never imply a diagnosis. Answers write into the M10-02 model on `OnboardingAnswers`.

- [ ] **M10-10** Adjacent-area probe, hard-capped (SPEC §14 #87 Tier 2): derive candidate adjacent muscle groups for each declared injury from `BodyAreaMuscleMapping` plus `SessionFocus.secondaryMuscles` (the closest thing to a synergist model the project has — no antagonist model exists), then probe **at most 2 adjacent groups × 1 exercise each** so onboarding length stays bounded. Unrelated groups are **not** probed during onboarding — they are Tier 3's job (M10-15). Findings feed the M10-11 derivation as a supporting signal, not as a separate injury declaration.

- [ ] **M10-11** Severity derivation + exhaustive tests (SPEC §14 #85): a pure, `nonisolated`, unit-tested mapping from `(quality, provoking-exercise count, adjacent findings)` → `severity: 1...5`, weighting **sharp** highest because it is the safety-critical signal: sharp on any of the three → **4–5** (hard-exclusion band, §14 #47); aching → **3** (supported/machine band, §14 #49c); dull → **1–2** (no primary targeting, light secondary). The number of provoking exercises moves the result toward the top of its band. The derived value is written into `OnboardingAnswers.injuries[area]` and flows into `PlanInput.injuries` / `PlanEngine.role` **unchanged** — do not touch `role`'s tested thresholds or `PlanInput.deterministicSeed` (which already folds pain levels in). Test the mapping table exhaustively (every quality × provoking-count × adjacent-finding combination) in `PluriTests`.

### Plan engine: injury-aware splits and day names

- [ ] **M10-12** Injuries become an input to the weekly split: `SessionFocus.split(daysPerWeek:experience:goal:)` currently ignores injuries entirely, so an injured user gets the same week *composition* as a healthy one and only the exercise substitutions differ. Add injuries to the signature and let the split avoid composing a day whose focus is dominated by a severity-4/5 area (e.g. don't schedule a Shoulders-primary day for a sharp shoulder), preferring a focus the user can actually train. Keep the existing `SessionFocusCode` set — **no new codes** (they are pinned by the Postgres CHECK in `supabase/migrations/20260725203245_plan_workouts_focus_check.sql` and the sync DTO). Preserve determinism (§14 #24) and keep the earned-Full-Body fallback (`PlanEngine.smallPoolFullBodyThreshold`, §14 #49d) as the last resort, not the first. Extend `PlanEngineTests` with injured personas per severity band.

- [ ] **M10-13** Injury-aware day-name override layer (SPEC §14 #86): a deterministic, pure, unit-tested title generator that produces a **specific, descriptive** session title for injured users — derived from the day's actual primary muscles plus the accommodation being made, e.g. "Chest & Triceps, Shoulder-Sparing", "Hips & Hamstrings Rebuild", "Upper Back Support & Core". Generic split labels ("Push", "Pull", "Legs") must never be shown to a user with a declared injury. Titles are written to the existing free-text **`plan_workouts.name`** column while the workout keeps its existing **`focus`** code for engine/DB/analytics compatibility — `name` and `focus` are separate columns, so this needs **zero migration**. No AI: the generator matches the engine's pure/deterministic character. Titles must fit the app's warm, non-clinical tone (design.md) and stay short enough for `PlanSessionRow`.

- [ ] **M10-14** Plumb the override through generation, persistence, and UI: `PlanEngine` currently sets `title: template.focus.displayTitle` — apply the M10-13 override there so the title flows through `GeneratedPlan` → `OnboardingSyncMapper` → `plan_workouts.name` → hydrate → `PlanSessionRow` / Home / Plan / Calendar / Workout Detail with no new plumbing. Verify every regeneration path carries it: Manage Plan (`PlanStore.applyManagePlan`), `PlanMutator.replacingRemainingWorkouts`, and the calendar add/move clone path (§14 #38). Healthy users keep `SessionFocus.displayTitle` verbatim. Note in the task that the project has **no `.xcstrings` catalog** — all copy is inline literals — so these strings are inline too (localization is M9).

### In-plan check-ins (the point-of-contact tiers)

- [ ] **M10-15** Tier 3 just-in-time prompt (SPEC §14 #87 — the answer to "the most effective point in the flow to ask whether specific exercises hurt"): on the **first occurrence** in the plan of an exercise that touches a flagged or adjacent area — including **secondary and never-declared** muscle groups — show a one-tap "how did that feel?" prompt immediately after the set is logged on the live Workout Screen. Rationale to carry into the copy and the code comment: the user has just performed the movement, so the answer is accurate and cheap, whereas asking about every muscle group during onboarding is unanswerable in the abstract and fatal to completion rate. Cap prompts per session (a small named constant), make every prompt **skippable** with no penalty, never block the set log, and write the answer as an `injury_assessments` row (source = in-workout) via the local-first path so an offline session never loses it (§13 cardinal rule).

- [ ] **M10-16** Per-workout structured area check on completion: on days that trained near a flagged area, `WorkoutCompletionView` offers a short, optional, area-specific check (better / same / worse for that area) **alongside** — not replacing — the existing free-text notes field, whose "How did it feel?" placeholder is currently the only "how do you feel" surface in the app and is never read back or fed to the engine. Answers persist as `injury_assessments` rows so they are actually consumable by M10-18. Skipping is free and silent.

### Weekly check-in, trend, and re-assessment

- [ ] **M10-17** Weekly injury check-in + its own notification family (SPEC §14 #88): at the plan week boundary — derived from `PlanMutator.weekNumber(containing:in:)` / `PlanEngine.orderedTrainingDates`, not a wall-clock week — ask one lightweight "how's your `<area>`?" (better / same / worse) per flagged area. Delivery reuses the M3-15 infrastructure but as a **separate family**: its own identifier prefix (e.g. `pluri.injury-checkin.`) so it only ever cancels its own requests (mirroring `WorkoutReminderCandidate.identifierPrefix`), its own user-scoped opt-in preference, permission requested only after explicit opt-in with gentle Settings guidance on denial (§14 #48b/c), and its own `…Reconciling` protocol alongside `WorkoutReminderReconciling` so the cancel-then-reschedule reconcile stays non-throwing into `PlanStore` (§14 #44f/#48f). Add the row to the Notifications page (SPEC §5.3).

- [ ] **M10-18** Derived trend signal: a pure, unit-tested reducer over the `injury_assessments` history for an area that answers "is this getting better, the same, or worse?" from the Tier 3 / per-workout / weekly observations, with an explicit **insufficient-data** state (no invented trend — same honesty rule as §14 #57c). This is the signal that triggers an early re-triage in M10-19 and the escalation copy in M10-22. It must not silently change the plan on its own.

- [ ] **M10-19** Full re-triage every **3 weeks** — or earlier when M10-18 reports a worsening trend (SPEC §14 #88): re-ask the **same** three primary exercises (M10-08's stable selection) plus the adjacent probe, recompute severity through M10-11, write the new `injury_assessments` rows, then **diff** against the previous assessment and present the change plainly ("your shoulder is reading milder than three weeks ago"). Reachable as a scheduled prompt and on demand from Manage Plan (M10-21). The re-triage itself never mutates the plan — that is M10-20.

- [ ] **M10-20** User-confirmed plan adaptation after re-triage (SPEC §14 #88): when the recomputed severity differs enough to change programming, offer regeneration of the **remaining** plan through the existing path — `PlanMutator.replacingRemainingWorkouts` / `PlanStore.replaceRemainingWorkouts`, atomic via the `replace_remaining_plan` RPC (migration `20260717194112`) — preserving completed/skipped history per §14 #39. Because `PlanInput.deterministicSeed` already folds pain levels in, an updated severity naturally yields a genuinely different plan with no seed changes. Regeneration is **always user-confirmed**, never silent (same posture as §14 #66c), and declining leaves the plan untouched while the new assessment is still recorded.

- [ ] **M10-21** Injuries become editable post-onboarding (SPEC §14 #88): `ManagePlanViewModel` (`Features/Plan/Manage/ManagePlanViewModel.swift`) deliberately treats injuries as non-editable engine inputs today — it passes `originalProfile.injuries` straight through — and there is **no** post-onboarding surface to edit them anywhere, which makes re-assessment meaningless. Add an injuries section that opens the same triage sub-flow (M10-05) for adding, re-assessing, or clearing an area, writes back through the M10-02 profile path, and routes any resulting regeneration through M10-20's confirmation. Respect §14 #45b: engine inputs read from the stored profile and a missing/invalid input fails gently with a validation message rather than guessing.

### Safety, accessibility & verification

- [ ] **M10-22** Safety, framing, and escalation (SPEC §14 #89): every injury surface states plainly that **Pluri is not physical therapy and does not diagnose**, aligned with the existing Ask Pluri disclaimer posture (§14 #66f). Add an escalation path recommending professional care when **sharp** pain is reported or when M10-18's trend worsens across consecutive re-assessments — offered as a kind suggestion, never a block or a scold. Copy review across the whole milestone against design.md's "this app never scolds": a user reporting more pain must never be made to feel they failed, and reduced or substituted work is framed as the plan doing its job. Also confirm the App Review posture (health-adjacent claims, no medical advice) before this ships.

- [ ] **M10-23** Accessibility + design-token pass on every new interactive surface (AGENTS §5): Dynamic Type on the triage screens, quality selector, in-workout prompt, weekly check-in, and Manage Plan injuries section; VoiceOver labels that read the exercise name **and** the pain question as one coherent element; 44pt minimum targets on the quality chips and the one-tap Tier 3 prompt; dark mode; and no ad-hoc colors — accents come from `PluriColor` tokens per design.md.

- [ ] **M10-24** Swift Testing coverage in `PluriTests`: the M10-11 severity mapping table exhaustively; M10-08's deterministic three-exercise selection (including the fewer-than-three fallback and stability across runs); the additive `InjuryJSON` round-trip **including legacy rows with no new fields**; injury-aware `SessionFocus.split` and the M10-13 title generator (no "Push/Pull/Legs" ever reaches an injured user); the M10-18 trend reducer including its insufficient-data state; the check-in reconciler's identifier-prefix isolation (it must not cancel workout reminders); and the M10-20 regeneration path preserving completed/skipped history.

- [ ] **M10-25** M10 UI QA: previews for healthy / single-injury / multi-injury / sharp-pain personas across the triage sub-flow, Q2 gate, in-workout prompt, weekly check-in, re-triage diff, and Manage Plan injuries section; confirm a healthy user's onboarding is **unchanged in length** and the progress bar still reads correctly after the 14 → 13 change; confirm an injured user never sees a generic split label anywhere in Home / Plan / Calendar / Detail; offline behavior for every check-in write; and full click-through Q2 → triage → plan → workout prompt → weekly check-in → re-triage → confirmed regeneration.

**Dependencies:** M10-01 → 02/03 → everything; 04 → 05 → 06/07 → 08 → 09 → 10 → 11; 11 → 12 → 13 → 14; 02+03 → 15/16; 15+16 → 17 → 18 → 19 → 20; 05+20 → 21; everything → 22/23/24/25.

**M10 exit check** (PLAN M10): injury capture happens at Q2 with a nested triage sub-flow and healthy users gain zero steps; Q7 is retired and the progress denominator is 13; the qualitative assessment derives a `1...5` severity that feeds the shipped `PlanEngine.role` thresholds unchanged; `profiles.injuries` grew additively and legacy rows still decode; dated observations persist under owner-only RLS; injured users' week composition and day names reflect their injuries with no new focus codes and no migration; the Tier 3 prompt fires at first occurrence, is capped and skippable, and survives offline; weekly check-ins ship as their own notification family without disturbing workout reminders; a 3-week re-triage recomputes severity, diffs it, and can adapt the remaining plan only with explicit confirmation; injuries are editable from Manage Plan; every injury surface carries non-medical framing with a sharp-pain / worsening-trend escalation path; build + Swift Testing suite pass; Dynamic Type / VoiceOver / 44pt / dark mode manually checked.

**Deferred out of M10 (don't build ahead):** per-exercise manual swap UI (whole-plan replacement via `replace_remaining_plan` remains the only substitution path); migrating plan generation to the server-side `generate-plan` Edge Function (PLAN §1.3 — still does not exist); AI-generated day titles or AI-driven triage (the title generator and severity derivation are deterministic by design); any antagonist/synergist model beyond what `SessionFocus.primaryMuscles` / `secondaryMuscles` already encodes; localization of the new copy (M9 — the project has no string catalog yet); Cardio / Flexibility / hybrid plans (v2).
