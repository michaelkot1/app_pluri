...
76. **Mid-workout Ask Pluri responsiveness (bug fix, 2026-07-27):** owner reported that asking a question during an active workout spiked CPU to ~99%, left the typed question sitting in the composer, and blocked follow-up questions even though the reply arrived. Fixes and the behavior they lock: (a) **Reachability is restartable.** `PathMonitorReachability.stop()` cancelled its `NWPathMonitor`, and a cancelled monitor never reports again — a stop → start cycle (chat sheet `onDisappear` → next send) froze `isOnline` at its last value, so `send()` could return early at the offline guard forever, keeping the draft in the box. `start()` now builds a fresh monitor; `stop()` releases it. This affects `SupabaseSyncEngine` too. (b) **Composer is never `.disabled()` while sending.** Disabling a focused `TextField` resigns first responder and can write its stale text back over the cleared draft. The Send button alone is gated by `canSend`; the user may compose the next question while the coach is thinking. (c) **`isSending` resets from a single `defer`**, and an in-flight send cancelled by the sheet going away releases the composer without an error banner (`CancellationError` is not "transport failed"). (d) **Failed sends keep the question in the transcript, not the composer** — draft is cleared on any attempted send; only the offline pre-check preserves it (#66d unchanged). (e) **One keyboard accessory per screen.** Every mounted `WorkoutExerciseCardView` registered a `ToolbarItemGroup(placement: .keyboard)`; only the focused card contributes one now. (f) **Exercise media is cheap while covered:** the exercise list is a `LazyVStack`, GIF frames decode off the main actor, `UIImageView` images are only re-assigned when the source changes (re-assigning an animated `UIImage` restarts it), and screens set `\.exerciseMediaAnimationEnabled = false` while a sheet covers them so GIFs hold a still frame. (g) **Timer/HealthKit observation is isolated** in `WorkoutTimerMetricsView`, so the 1 Hz tick and incoming samples no longer invalidate the exercise list. HealthKit-driven updates are still unthrottled (samples are infrequent); revisit only if profiling shows otherwise.
